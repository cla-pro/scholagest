...
public class OntologyMerger {
	...
	public void mergeOntologyWithRestObject(RestObject restClassInfo,
			Map<String, OntologyElement> ontology) throws Exception {
		Map<String, RestProperty> properties = restClassInfo.getProperties();
		
		for (String propertyName : properties.keySet()) {
			RestProperty property = properties.get(propertyName);
			
			findOntologyElementAndUpdateProperty(ontology, propertyName,
					property);
		}
	}

	private void findOntologyElementAndUpdateProperty(
			Map<String, OntologyElement> ontology, String propertyName,
			RestProperty property) throws Exception {
