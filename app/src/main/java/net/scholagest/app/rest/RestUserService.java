...
public class RestUserService {
	...
	public String authenticate(
		...
		ITransaction transaction = this.database.getTransaction(
				ScholagestNamespace.scholagestKeyspace);
		try {
			transaction.insert(token,
					ScholagestNamespace.pSessionExpirationDate,
					new DateTime().plusHours(2).toString("yyyy-MM-dd HH:mm:ss"),
					null);
		...
		return new JsonObject("token", token,
				"nextpage", "http://localhost:8080/scholagest-app/services/user/getPage").toString();
	}
	...
	public String getPage(@QueryParam("token") String token) {
		...
		builder.append("<div id=\"tabMenu\" data-dojo-type=\"dijit.layout.TabContainer\" "
		...
		String basePage = loadFile("html" + File.separatorChar
							+ "base.html");
