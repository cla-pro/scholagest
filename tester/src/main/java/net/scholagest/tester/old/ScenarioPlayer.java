...
public class ScenarioPlayer {
    ...
    // LogManager.getLogger(ScenarioPlayer.class.getName());
    ...
    // new CallHandler(responseAnalyzer,
    // placeholder).handleCallAndException(scenario.getBaseURL(), call);
    // }
    //
    ...
    // }
    //
    // private TestResultStatus areAllTestResultsOk(List<TestResult>
    // testResultList) {
    ...
    // LOG_RESULT.info(String.format("========================== Test: %s",
    // testResult.getTestName()));
    //
    // for (CallResult result : testResult.getResults()) {
    // LOG_RESULT.info(String.format("-------------- Call id: %s",
    // result.getCall().getId()));
    // LOG_RESULT.info(String.format("Url: %s", result.getCall().getUrl()));
    // LOG_RESULT.info(String.format("Parameters: %s",
    // result.getCall().getParameters()));
    // LOG_RESULT.info(String.format("Result status: %s", result.getStatus()));
    // try {
    // LOG_RESULT.info(String.format("Response: %s",
    // result.getContentExchange().getResponseContent()));
    // } catch (UnsupportedEncodingException e) {
    // e.printStackTrace();
    // }
    //
    // for (FieldResult fieldResult : result.getFieldResults()) {
    // System.out.println(String.format("%10s | %50s | %50s | %50s",
    // fieldResult.getStatus().name(), fieldResult.getExpectedValue(),
    // fieldResult.getReceivedValue(), fieldResult.getPath()));
    // }
    // }
    //
    // LOG_RESULT.info(String.format("========================== Result: %s",
    // testResult.getTestStatus().name()));
    // LOG_RESULT.info("==============================================================================");
    // }
}
