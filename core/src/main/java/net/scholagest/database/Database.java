...
public class Database implements IDatabase {
	...
	public void startup() {
		...
		this.cluster = HFactory.getOrCreateCluster(
				this.databaseConfiguration.getClusterName(),
				this.databaseConfiguration.getHostConfigurator());
	}
	...
	public ITransaction getTransaction(String keyspaceName) {
		...
		ColumnFamilyTemplate<String, String> columnFamilyTemplate =
				new ThriftColumnFamilyTemplate<>(keyspace, COLUMN_FAMILY_NAME,
	...
	private Keyspace getOrCreateKeyspace(String keyspaceName) {
		if (cluster.describeKeyspace(keyspaceName) == null) {
			...
			KeyspaceDefinition newKeyspace = HFactory.createKeyspaceDefinition(
					...
					this.databaseConfiguration.getReplicationFactor(),
					Arrays.asList(cfDef));
	...
	private class Transaction implements ITransaction {
		...
		public Transaction(Keyspace keyspace,
		...
		public Set<String> getColumns(String key) throws DatabaseException {
			SliceQuery<String, String, String> query =
					HFactory.createSliceQuery(keyspace, StringSerializer.get(),
					    StringSerializer.get(), StringSerializer.get()).
					    setKey(key).setColumnFamily(COLUMN_FAMILY_NAME);

			ColumnSliceIterator<String, String, String> iterator =
			    new ColumnSliceIterator<String, String, String>(
			    		query, null, "\uFFFF", false);
